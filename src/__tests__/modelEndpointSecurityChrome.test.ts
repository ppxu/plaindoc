...
describe("model endpoint security chrome", () => {
  it("surfaces endpoint security checks before model sending", () => {
    ...
    expect(modelSettingsPanelSource).toContain("const hasDocumentText = Boolean(documentText.trim());");
    ...
    expect(modelSettingsPanelSource).toContain(
    ...
    expect(modelSettingsPanelSource).toContain("粘贴、上传或选择样例正文后才能确认发送给模型服务。");
