...
AI-enhanced mode is optional. It sends document text only after the user enables AI settings and confirms **本次允许发送正文给模型服务** for the current document/model destination. Blank or whitespace-only document text cannot be confirmed for model sending and does not show a sendable-text preview. PlainDoc sends at most 12,000 characters from the beginning and ending portions of the document text to the configured OpenAI-compatible endpoint; the full text is still analyzed locally. Before confirmation, PlainDoc shows a read-only preview of the exact text that will be sent to the configured model service.
